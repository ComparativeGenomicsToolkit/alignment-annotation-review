...
% "chromatin configuration": maybe cite the chromatin state segmentation paper that was released with ENCODE (iirc)? everything else has a citation
...
% sadly there is no 200 mammals paper yet. should we cite the grant or something? It looks a little weird without a citation
...
% Could/should phyloCSF be mentioned here? It doesn't produce gene models (does it?), but it feels like it's very much in the same spirit as the above tools.
...
% I get what you're going for here, but right now the above sentence feels a little disconnected from everything else. This might need another bit here explicitly connecting the above sentence to why transcript projection is a good idea--basically just making the point that *because* you have to sequence across tens of tissues and/or conditions to get a full annotation set, it's prohibitively expensive to do all tissues for every organism and it makes sense to share the resulting data across organisms.
...
% biggest benefit relative to what? I'm not sure I understand
...
% I agree with the general sentiment, but I think the sentence below is too extreme. Plenty of people use Ensembl genes, which are comparatively annotated!
...
% The below paragraph needs to cite a lot of stuff, but I'm assuming the citations are blocked on the CAT/great ape papers being published.
...
% TODO: cite the zebrafinch pacbio assembly paper?
